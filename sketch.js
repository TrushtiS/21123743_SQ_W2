...
let platforms = [
  ...
  { x: 340, y: 280, w: 140, h: 16 }, // centre platform (moved)
  ...
  { x: 200, y: 200, w: 140, h: 16, type: 'slippery' } // new playform which is slipery
];
...
function preload(){
  ...
  cImage = loadImage("assets/images/images.png"); 
...
function resolvePlatformCollisions() {
  for (let i = 0; i < platforms.length; i++) {
    ...
    if (overlapsHorizontally && landingOnTop) {
      ...
      //overwrites to mkae block slippery
      if(p.type === 'slippery'){
        player.vx *= 1.50;
      }
    }
...
function drawPlatforms() {
  ...
  for (let i = 0; i < platforms.length; i++) {
    ...
    rect(p.x, p.y, p.w, p.h, 7); // rounded corners
  }
}
...
function drawPlayer() {
  ...
  if (cImage.vx < 0) {
    ...
    image(cImage, -player.x, player.y, player.r * 2, player.r * 2);
  } else {
    image(cImage, player.x, player.y, player.r * 2, player.r * 2);
  }
  pop();
